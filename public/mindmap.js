...
function renderCategoryChips() {
  const categories = [...new Set(allPeople.flatMap(p => p.category))];
...
function drawMindmap() {
  ...
  const filteredPeople = allPeople.filter(person => {
    ...
    if (person.relationshipStrength < minStrength) return false;
  ...
  const simulation = d3.forceSimulation(filteredPeople)
    .force("link", d3.forceLink(filteredEdges).id(d => d.id).distance(95).strength(0.24))
    .force("charge", d3.forceManyBody().strength(-200))
    .force("center", d3.forceCenter(width / 2, height / 2))
    .force("collide", d3.forceCollide().radius(d => Math.max(16, 12 + d.relationshipStrength * 2)));
  ...
  const node = svg.append("g")
    ...
    .data(filteredPeople)
    .join("circle")
    .attr("r", d => Math.max(6, Math.min(24, 6 + d.relationshipStrength * 1.8)))
    ...
    .on("mousemove", (event, d) => {
      showTooltip(event, `
        ...
        Category: ${d.category.join(", ") || "-"}<br>
        Layer: ${d.layer || "-"}<br>
        Client Status: ${d.clientStatus || "-"}<br>
        Relationship Strength: ${d.relationshipStrength}<br>
